...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch players if user is a coach
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {isLoading
                  ...
                  : `${gameStats[
                      gameStats.length - 1
                    ]?.stats.hittingPercentage.toFixed(1)}%`}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {isLoading
                  ...
                  : `${gameStats[
                      gameStats.length - 1
                    ]?.stats.passingPercentage.toFixed(1)}%`}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {isLoading
                  ...
                  : `${gameStats[
                      gameStats.length - 1
                    ]?.stats.servingEfficiency.toFixed(1)}%`}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {isLoading
                  ...
                  : `${gameStats[
                      gameStats.length - 1
                    ]?.stats.blockingEfficiency.toFixed(1)}%`}
              </div>
            </CardContent>
          </Card>
        </div>
                  ...
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">
                        {game.sets?.filter(
                          (set) => set.homeScore > set.opponentScore
                        ).length || 0}{" "}
                        -{" "}
                        {game.sets?.filter(
                          (set) => set.homeScore < set.opponentScore
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          <div>
                            ...
                            <span className="ml-2 font-medium text-gray-900 dark:text-white">
                              {set.stats?.hittingPercentage?.toFixed(1) ||
                                "0.0"}
                              %
                            </span>
                          </div>
                          <div>
                            ...
                            <span className="ml-2 font-medium text-gray-900 dark:text-white">
                              {set.stats?.passingPercentage?.toFixed(1) ||
                                "0.0"}
                              %
                            </span>
                          </div>
                          <div>
                            ...
                            <span className="ml-2 font-medium text-gray-900 dark:text-white">
                              {set.stats?.servingEfficiency?.toFixed(1) ||
                                "0.0"}
                              %
                            </span>
                          </div>
                          <div>
                            ...
                            <span className="ml-2 font-medium text-gray-900 dark:text-white">
                              {set.stats?.blockingEfficiency?.toFixed(1) ||
                                "0.0"}
                              %
                            </span>
                          </div>
